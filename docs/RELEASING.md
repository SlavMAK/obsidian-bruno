...
## Почему main.js такой большой

Вью открывается через `srcdoc`-iframe, который читает `webview/index.html` с
диска и подтягивает ленивые чанки rsbuild по `app://`. То есть плагину нужен
каталог `webview/` рядом с `main.js` — а каталог Obsidian ставит и обновляет
только `main.js`, `manifest.json` и `styles.css`.

Поэтому сборка зашивает весь `webview/` внутрь `main.js` одним gzip-блобом
(7.5 MB → 3.9 MB base64, `esbuild.webview-embed.mjs`), а при первой загрузке
`src/obsidian/webview-assets.ts` распаковывает его рядом с `main.js`. Отсюда
размер `main.js` ~16 MB.

Распаковка идемпотентна: в `webview/.bruno-build-id` лежит хеш блоба, и если он
совпадает — плагин ничего не трогает. При несовпадении каталог сносится целиком
(имена чанков содержат хеш, поэтому осиротевший файл от прошлой версии иначе
остался бы навсегда). В zip стемп уже лежит, так что установка архивом
распаковку пропускает.

В watch-режиме (`npm run dev`) блоб не зашивается, чтобы `npm run dev:webview`
мог перезаписывать `webview/` вживую.

Инвариант закрыт тестом `npm run test:webview`: он собирает настоящий
распаковщик настоящим плагином, распаковывает во временный каталог и сверяет
все 114 файлов побайтово, плюс проверяет идемпотентность и вычистку старых
файлов.
