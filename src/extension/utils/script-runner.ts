...
// Runtime variables live in each webview's Redux store, so a leaf opened after
// a script ran would start empty. Keep the latest set per collection to seed it
// (see shell.ts seedRuntimeVariables).
const runtimeVariablesByCollection = new Map<string, unknown>();
export const getRuntimeVariables = (collectionUid: string): unknown => runtimeVariablesByCollection.get(collectionUid);
...
const emitScriptVariableUpdates = (
  ...
  // Every editor leaf has its own Redux store, so runtime variables set by a
  // script (bru.setVar) must reach all of them, not just the sender — otherwise
  // a token fetched in one tab is invisible to the request in the next.
  if (result.runtimeVariables) {
    runtimeVariablesByCollection.set(context.collectionUid, result.runtimeVariables);
  }
