...
// Electron's renderer cannot spawn node worker_threads, so the "via worker"
// variant parses in-process like the plain one.
export const parseRequestViaWorker = async (content: string, options?: ParseOptions): Promise<any> =>
  parseRequestFile(stripDirtyMarker(content), options);
