...
// Obsidian never opens .bru files in a competing editor, so writes always go
// straight to disk (the VS Code build routed them through a dirty-state manager).
const isDocumentRegistered = (_pathname: string): boolean => false;
const writeFileViaVSCode = async (_pathname: string, _content: string): Promise<void> => {};
...
const registerCollectionIpc = (watcher: CollectionWatcherInterface): void => {
  ...
  registerHandler('renderer:new-request', async (args) => {
    ...
    try {
      ...
      const content = await stringifyRequest(request, { format });
      ...
      const writeItems = async (items: any[], parentDir: string) => {
        ...
        for (const item of items) {
          ...
          if (!isRequestType && (item.type === 'folder' || (item.items && Array.isArray(item.items) && item.items.length > 0))) {
            ...
            try {
              const requestContent = await stringifyRequest(item, { format });
